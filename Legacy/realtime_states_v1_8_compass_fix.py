"""
S02 — Realtime-States v1.8 Compass Fix (Corrected)
==================================================

KRITISCHE FIX: EMA update met window_direction * window_confidence, niet direct dt_ab.

Het Inzicht:
-----------
v1.5 gebruikt InertialCompassState die:
1. Een sliding window bijhoudt van dt_ab waarden
2. Window direction + confidence berekent uit dat window
3. EMA update doet met: signed = window_direction * window_confidence

Dit betekent dat bij mono tiles (nA=1, nB=0):
- Er is geen NIEUWE dt_ab
- MAAR het window "herinnert" oude dt_ab waarden
- Het window geeft nog steeds een richting door
- De EMA blijft updaten richting die remembered direction

v1.7/v1.8-bug:
- EMA werd direct met dt_ab geüpdatet
- Bij mono tiles: geen update → score bleef stabiel
- Dit was FUNDAMENTEEL ANDERS dan v1.5

v1.8 Fix:
- Window houdt sliding window van dt_ab (inclusief None waarden)
- Window berekent direction + confidence uit ALLEEN de geldige dt_ab waarden
- EMA update ALTIJD met window_direction * window_confidence
- Mono tiles: window herinnert richting, EMA blijft updaten
"""
...
class TilesState:
    """v1.8: Tiles met pure stereo marking voor dt_ab."""
    ...
    def _make_tile(self, idx: int, 
        ...
        # v1.8: PURE STEREO CHECK
        # dt_ab alleen berekenen bij exact nA=1 en nB=1
...
# === Compass v1.8 — Corrected with Window Memory =============================
...
class CompassAdapter:
    """
    v1.8 FIXED: Compass met window memory zoals v1.5.
    
    Architectuur:
    1. Sliding window houdt dt_ab waarden bij (inclusief None)
    2. Window berekent direction + confidence uit GELDIGE dt_ab waarden
    3. EMA update ALTIJD met: signed = window_direction * window_confidence
    4. Bij mono tiles: window herinnert richting, EMA blijft updaten
    
    Dit is EXACT hoe v1.5/InertialCompassState werkt!
    """
    ...
    def __init__(self, profile: PipelineProfile):
        ...
        # Sliding window van dt_ab waarden (kan None bevatten)
        self._dt_ab_window: deque = deque(maxlen=self._window_tiles)
    ...
    def _direction_to_signed_score(self, direction: str, confidence: float) -> float:
        """Convert window direction + confidence naar signed score."""
        if direction == "CW":
            return +confidence
        elif direction == "CCW":
            return -confidence
    ...
    def feed_tile(self, tile: Dict[str, Any], tile_duration_us: Optional[float]) -> CompassSnapshot:
        """
        v1.8 FIXED: Verwerk tile zoals v1.5/InertialCompassState.
        
        1. Voeg dt_ab toe aan window (ook None voor mono tiles)
        2. Bereken window direction + confidence uit geldige dt_ab's
        3. Update EMA met window_direction * window_confidence
        """
        self._tile_count += 1
        dt_ab = tile.get("dt_ab_us")  # Kan None zijn voor mono tiles
        
        # 1. Voeg toe aan sliding window (ook None)
        self._dt_ab_window.append(dt_ab)
        
        # 2. Bereken window direction + confidence uit GELDIGE dt_ab's
        valid_dts = [d for d in self._dt_ab_window if d is not None]
        
        window_dir = "UNDECIDED"
        window_conf = 0.0
        
        if valid_dts:
            cw_count = sum(1 for d in valid_dts if d > 0)
            ccw_count = sum(1 for d in valid_dts if d < 0)
            total = len(valid_dts)
            ...
            # else: UNDECIDED met conf 0
        
        # 3. EMA update ALTIJD met window signaal (zoals v1.5!)
        ...
        # 4. Direction decision met hysterese
        ...
        return CompassSnapshot(
            ...
            window_meta={"valid_dts": len(valid_dts), "total_window": len(self._dt_ab_window)},
            global_meta={},
        )
...
class MovementBodyV3:
    """v1.8: MovementBody met profile-based parameters."""
...
# === RealtimePipeline v1.8 ===================================================

class RealtimePipeline:
    """
    S02-Realtime-States v1.8 — Compass Fix (Corrected)
    
    Nu met correcte window-based EMA zoals v1.5.
    """
    ...
    def feed_event(self, ev: Dict[str, Any]) -> RealtimeSnapshot:
        ...
        for tile in tiles:
            ...
            # v1.8: ALLE tiles met data krijgen compass update
            if has_data:
